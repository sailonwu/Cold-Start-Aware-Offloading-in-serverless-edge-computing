...
NUM_EPISODE = 1000   #迭代的轮次数量 

n_iot = 4    #本地iot设备的处理能力
n_edge = 2   # 边缘服务器的数量
n_cloud = 1  #云服务器的数量
...
pull_uplink_rate_iot =   14.000000 
...
def compute_cost(observation, node_type, local_index,j,edge_idx=0, cloud_idx=0):
    #print("系统观测值",observation[:n_task_type * 5 +1])
    if node_type == 'local':
        per_task_wait_iot_comp_count_cpu_cycle_sum = sum(v for v in observation[n_task_type:n_task_type*2] if v > 0)
        local_need_time = per_task_wait_iot_comp_count_cpu_cycle_sum / comp_cap_iot[local_index]
        #print("all_task_wait_iot_comp_count_sum",local_need_time1,local_need_time)
        if local_need_time > 0 :
            cold_start_delay_sum = 0
        else:
            cold_start_delay_sum = task[local_index][j][0]['data_size'] / pull_uplink_rate_iot
        local_need_time_sum = local_need_time+ cold_start_delay_sum
        local_need_enegry = esc * (comp_cap_iot[local_index] ** 2) * local_need_time_sum
        #print(local_need_time_sum,local_need_enegry)
        cost =  local_need_time_sum
        
    elif node_type == 'edge':
        tran_wait_time = observation[ n_task_type * 5 + 2] /tran_edge_cap_iot[local_index][0]
        # 通信 + 边缘剩余计算
        if observation[ n_task_type * 5 + 6 + n_edge + n_cloud + 2] > 0:
            compute_temp_time1  = observation[ n_task_type * 5 + 6 + n_edge + n_cloud + 2] / comp_cap_edge[0]
        else:
            compute_temp_time1 = 0
        if observation[ n_task_type * 8 + 6 + n_edge + n_cloud + 6] > 0:
            compute_temp_time2  = observation[ n_task_type * 8 + 6 + n_edge + n_cloud + 6] / comp_cap_edge[0]
        else:
            compute_temp_time2 = 0
        compute_temp_time = max(compute_temp_time1,compute_temp_time2,0)
        if compute_temp_time > 0 :
            cold_start_delay_sum = 0
        else:
            cold_start_delay_sum = task[local_index][j][0]['data_size'] / pull_uplink_rate_edge
        edge_need_time_sum = max(tran_wait_time,compute_temp_time) + cold_start_delay_sum
        cost = edge_need_time_sum
    elif node_type == 'cloud':
        tran_wait_time = observation[ n_task_type * 5 + 2] /tran_cloud_cap_iot[local_index][0]
        compute_temp_time  = observation[ n_task_type * 11 + 6 + n_edge + n_cloud + 10] / comp_cap_cloud[0]
        
        if compute_temp_time > 0 :
            cold_start_delay_sum = 0
        else:
            cold_start_delay_sum = task[local_index][j][0]['data_size'] / pull_uplink_rate_cloud
        
        cost = max(tran_wait_time,compute_temp_time) + cold_start_delay_sum
    return cost

def make_offload_decision(observation_all):
    decisions = []
    for i in range(n_iot):
        obs = observation_all[i]
        cost_local = compute_cost(obs, 'local',i)
        cost_edge = compute_cost(obs, 'edge',i)
        cost_cloud = compute_cost(obs, 'cloud',i)
...
def train(iot_RL_list, NUM_EPISODE):
    ...
    for episode in range(NUM_EPISODE):
        ...
        for j in range(n_time):
            ...
            for iot_index in range(n_iot):
                ...
                if task[iot_index][j][0]['function_id'] != -1:
                    ...
                    obs = np.squeeze(observation_all[iot_index, :])
                    cost_local = compute_cost(obs, 'local',iot_index,j)
                    cost_edge = compute_cost(obs, 'edge',iot_index,j)
                    cost_cloud = compute_cost(obs, 'cloud',iot_index,j)
                    costs = {
                        'local': cost_local,
                        'edge': cost_edge,
                        'cloud': cost_cloud
                    ...
                    if best_choice == "local":
                        action[iot_index][j][0] = 0
                    elif best_choice == "cloud":
                        action[iot_index][j][0] = n_actions - 1
                    else:
                        action[iot_index][j][0] = np.random.randint(1, n_actions-1)
        ...
        np.savetxt(f'./结果分析/奖励-Cost_choise_4_arrive_001_learning_99_discount_64_batchsize_max_delay_4_iot.csv', reward_list_record, delimiter=',')
        np.savetxt(f'./结果分析/丢失任务总数-Cost_choise_4_arrive_001_learning_99_discount_64_batchsize_max_delay_4_iot.csv', unfinish_total_list_record, delimiter=',')
        np.savetxt(f'./结果分析/任务总时延-Cost_choise_4_arrive_001_learning_99_discount_64_batchsize_max_delay_4_iot.csv', delay_list_record, delimiter=',')
        np.savetxt(f'./结果分析/任务总能耗-Cost_choise_4_arrive_001_learning_99_discount_64_batchsize_max_delay_4_iot.csv', energy_list_record, delimiter=',')
        np.savetxt(f'./结果分析/任务总花费-Cost_choise_4_arrive_001_learning_99_discount_64_batchsize_max_delay_4_iot.csv', expense_list_record, delimiter=',')
        np.savetxt(f'./结果分析/任务是冷启动-Cost_choise_4_arrive_001_learning_99_discount_64_batchsize_max_delay_4_iot.csv', cold_count_list_record, delimiter=',')
        np.savetxt(f'./结果分析/任务是热启动-Cost_choise_4_arrive_001_learning_99_discount_64_batchsize_max_delay_4_iot.csv', warm_count_list_record, delimiter=',')
        np.savetxt(f'./结果分析/成功任务总数-Cost_choise_4_arrive_001_learning_99_discount_64_batchsize_max_delay_4_iot.csv', sucess_count_list_record, delimiter=',')
        np.savetxt(f'./结果分析/卸载结果iot_-Cost_choise_4_arrive_001_learning_99_discount_64_batchsize_max_delay_4_iot.csv', action_record_iot, delimiter=',')
        np.savetxt(f'./结果分析/卸载结果edge_-Cost_choise_4_arrive_001_learning_99_discount_64_batchsize_max_delay_4_iot.csv', action_record_edge, delimiter=',')
        np.savetxt(f'./结果分析/卸载结果cloud_-Cost_choise_4_arrive_001_learning_99_discount_64_batchsize_max_delay_4_iot.csv', action_record_cloud, delimiter=',')
        np.savetxt(f'./结果分析/任务执行标志0-总数-Cost_choise_4_arrive_001_learning_99_discount_64_batchsize_max_delay_4_iot.csv', unfinish_total_list_record0, delimiter=',')
        np.savetxt(f'./结果分析/任务执行标志1--Cost_choise_4_arrive_001_learning_99_discount_64_batchsize_max_delay_4_iot.csv', unfinish_total_list_record1, delimiter=',')
        np.savetxt(f'./结果分析/任务执行标志2--Cost_choise_4_arrive_001_learning_99_discount_64_batchsize_max_delay_4_iot.csv', unfinish_total_list_record2, delimiter=',')
        np.savetxt(f'./结果分析/任务执行标志3--Cost_choise_4_arrive_001_learning_99_discount_64_batchsize_max_delay_4_iot.csv', unfinish_total_list_record3, delimiter=',')
        np.savetxt(f'./结果分析/任务执行标志4--Cost_choise_4_arrive_001_learning_99_discount_64_batchsize_max_delay_4_iot.csv', unfinish_total_list_record4, delimiter=',')
        np.savetxt(f'./结果分析/任务执行标志5--Cost_choise_4_arrive_001_learning_99_discount_64_batchsize_max_delay_4_iot.csv', unfinish_total_list_record5, delimiter=',')
        np.savetxt(f'./结果分析/网络选择动作数量--Cost_choise_4_arrive_001_learning_99_discount_64_batchsize_max_delay_4_iot.csv', choose_action_by_net, delimiter=',')
        np.savetxt(f'./结果分析/随机选择动作数量--Cost_choise_4_arrive_001_learning_99_discount_64_batchsize_max_delay_4_iot.csv', choose_action_by_random, delimiter=',')
        '''if episode == 10:
            return'''
...
if __name__ == "__main__":
    ...
    with open('任务数txt\\record-Cost_choise_4_arrive_001_learning_99_discount_64_batchsize_max_delay_4_iot.txt', 'w') as f:
