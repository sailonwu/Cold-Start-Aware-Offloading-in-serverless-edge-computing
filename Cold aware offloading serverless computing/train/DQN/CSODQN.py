...
def train(iot_RL_list, NUM_EPISODE):
    ...
    for episode in range(NUM_EPISODE):
        ...
        np.savetxt(f'./结果分析/奖励-Dou_Due_4_arrive_0001_learning_99_discount_64_batchsize_max_delay_6_iot_priority.csv', reward_list_record, delimiter=',')
        np.savetxt(f'./结果分析/丢失任务总数-Dou_Due_4_arrive_0001_learning_99_discount_64_batchsize_max_delay_6_iot_priority.csv', unfinish_total_list_record, delimiter=',')
        np.savetxt(f'./结果分析/任务总时延-Dou_Due_4_arrive_0001_learning_99_discount_64_batchsize_max_delay_6_iot_priority.csv', delay_list_record, delimiter=',')
        np.savetxt(f'./结果分析/任务总能耗-Dou_Due_4_arrive_0001_learning_99_discount_64_batchsize_max_delay_6_iot_priority.csv', energy_list_record, delimiter=',')
        np.savetxt(f'./结果分析/任务总花费-Dou_Due_4_arrive_0001_learning_99_discount_64_batchsize_max_delay_6_iot_priority.csv', expense_list_record, delimiter=',')
        np.savetxt(f'./结果分析/任务是冷启动-Dou_Due_4_arrive_0001_learning_99_discount_64_batchsize_max_delay_6_iot_priority.csv', cold_count_list_record, delimiter=',')
        np.savetxt(f'./结果分析/任务是热启动-Dou_Due_4_arrive_0001_learning_99_discount_64_batchsize_max_delay_6_iot_priority.csv', warm_count_list_record, delimiter=',')
        np.savetxt(f'./结果分析/成功任务总数-Dou_Due_4_arrive_0001_learning_99_discount_64_batchsize_max_delay_6_iot_priority.csv', sucess_count_list_record, delimiter=',')
        np.savetxt(f'./结果分析/卸载结果iot_-Dou_Due_4_arrive_0001_learning_99_discount_64_batchsize_max_delay_6_iot_priority.csv', action_record_iot, delimiter=',')
        np.savetxt(f'./结果分析/卸载结果edge_-Dou_Due_4_arrive_0001_learning_99_discount_64_batchsize_max_delay_6_iot_priority.csv', action_record_edge, delimiter=',')
        np.savetxt(f'./结果分析/卸载结果cloud_-Dou_Due_4_arrive_0001_learning_99_discount_64_batchsize_max_delay_6_iot_priority.csv', action_record_cloud, delimiter=',')
        np.savetxt(f'./结果分析/任务执行标志0-总数-Dou_Due_4_arrive_0001_learning_99_discount_64_batchsize_max_delay_6_iot_priority.csv', unfinish_total_list_record0, delimiter=',')
        np.savetxt(f'./结果分析/任务执行标志1--Dou_Due_4_arrive_0001_learning_99_discount_64_batchsize_max_delay_6_iot_priority.csv', unfinish_total_list_record1, delimiter=',')
        np.savetxt(f'./结果分析/任务执行标志2--Dou_Due_4_arrive_0001_learning_99_discount_64_batchsize_max_delay_6_iot_priority.csv', unfinish_total_list_record2, delimiter=',')
        np.savetxt(f'./结果分析/任务执行标志3--Dou_Due_4_arrive_0001_learning_99_discount_64_batchsize_max_delay_6_iot_priority.csv', unfinish_total_list_record3, delimiter=',')
        np.savetxt(f'./结果分析/任务执行标志4--Dou_Due_4_arrive_0001_learning_99_discount_64_batchsize_max_delay_6_iot_priority.csv', unfinish_total_list_record4, delimiter=',')
        np.savetxt(f'./结果分析/任务执行标志5--Dou_Due_4_arrive_0001_learning_99_discount_64_batchsize_max_delay_6_iot_priority.csv', unfinish_total_list_record5, delimiter=',')
        np.savetxt(f'./结果分析/网络选择动作数量--Dou_Due_4_arrive_0001_learning_99_discount_64_batchsize_max_delay_6_iot_priority.csv', choose_action_by_net, delimiter=',')
        np.savetxt(f'./结果分析/随机选择动作数量--Dou_Due_4_arrive_0001_learning_99_discount_64_batchsize_max_delay_6_iot_priority.csv', choose_action_by_random, delimiter=',')
        '''if episode == 10:
            return'''
...
class DeepQNetwork:
    ...
    def store_transition(self, s, a, r, s_):
        ...
        # Calculate TD error
        td_q_eval = self.sess.run(self.q_eval, feed_dict={self.s: s[np.newaxis, :]})   #评估网络
        td_q_next = self.sess.run(self.q_next, feed_dict={self.s_: s_[np.newaxis, :]})  #目标网络
        q_eval_next = self.sess.run(self.q_eval, feed_dict={self.s: s_[np.newaxis, :]})
        max_act_next = np.argmax(q_eval_next, axis=1)
        selected_q_next = td_q_next[0, max_act_next[0]]
        td_q_target = td_q_eval.copy()
        td_q_target[0, int(a)] = r + self.gamma * selected_q_next
        td_error = abs(td_q_target[0, int(a)] - td_q_eval[0, int(a)]) + 0.0000000000001
        if np.isnan(td_error) or np.isinf(td_error):
            #print(f"Invalid TD error: {td_error}. Setting to 0.")
            td_error =  0.0000000000001
        self.priority[index] = td_error
    ...
    def _build_net(self):
        ...
        self.is_weights = tf.placeholder(tf.float32, [None], name='is_weights')  # importance sampling weights
        ...
        with tf.variable_scope('loss'):
            ...
            squared_diff = tf.squared_difference(self.q_target, self.q_eval)
            is_weights_expanded = tf.expand_dims(self.is_weights, axis=1)
            weighted_squared_diff = is_weights_expanded * squared_diff
            self.loss = tf.reduce_mean(weighted_squared_diff)
    ...
    def learn(self):
        ...
        if self.memory_counter > self.memory_size:
            ...
            sample_size = self.memory_size
        else:
            sample_size = self.memory_counter
            #sample_index = np.random.choice(self.memory_counter, size=self.batch_size)
        
        # Normalize priorities to create sampling probabilities
        priority_sum = np.sum((self.priority[:sample_size])** self.alpha)
        sampling_probabilities = ((self.priority[:sample_size])** self.alpha) / priority_sum
        # Sample indices based on probabilities
        sample_indices = np.random.choice(sample_size, self.batch_size, p=sampling_probabilities)
        batch_memory = self.memory[sample_indices, :]
        # Compute importance sampling weights
        is_weights = ((1 / sample_size) * (1 / sampling_probabilities[sample_indices])) ** self.beta
...
if __name__ == "__main__":
    ...
    with open('任务数txt/record-Dou_Due_4_arrive_0001_learning_99_discount_64_batchsize_max_delay_6_iot_priority.txt', 'w') as f:
